import { buildCorsHeaders, handleApplyHttpRequest, parseRequestBody } from './lib.mjs'
...
export async function main(args) {
  ...
  try {
    ...
    return await handleApplyHttpRequest({ method, origin, body }, env)
  } catch (error) {
    console.error('apply-submit error:', error)
    return {
      statusCode: 500,
      headers: buildCorsHeaders(origin, env.ALLOWED_ORIGINS),
      body: JSON.stringify({ error: 'Unable to submit your application. Please try again.' }),
    }
  }
}
